...
Roland707M.init = function () {
  Roland707M.shiftButton = function (channel, control, value, _status, _group) {
    Roland707M.deck
      .concat(Roland707M.effectUnit, [
        Roland707M.sampler,
        Roland707M.browseEncoder,
      ])
      .forEach(
        value
          ? function (module) {
  ...
  for (let i = 0; i <= 1; i++) {
    ...
    Roland707M.effectUnit[i].dryWetKnob.input = function (
      channel,
      control,
      value,
      _status,
    ...
    for (let j = 1; j <= 4; j++) {
      ...
      Roland707M.effectUnit[i].enableOnChannelButtons["Channel" + j].midi = [
        0x98 + i,
        0x04 + j,
      ];
    }
    Roland707M.effectUnit[i].enableOnChannelButtons.addButton(
      Roland707M.trsGroup,
    );
    Roland707M.effectUnit[i].enableOnChannelButtons[Roland707M.trsGroup].midi =
      [0x98 + i, 0x09];
    Roland707M.effectUnit[i].enableOnChannelButtons[Roland707M.trsGroup].input =
      function (channel, control, value, status, _group) {
        ...
        if (this.isPress(channel, control, value, status)) {
          ...
          for (let j = 1; j <= 16; j++) {
            engine.setValue(
              this.group,
              "group_[Sampler" + j + "]_enable",
              enabled,
            );
          }
        }
      };
    Roland707M.effectUnit[i].enableOnTrsButton =
      Roland707M.effectUnit[i].enableOnChannelButtons[Roland707M.trsGroup];
    ...
    this.onButtonEvent = function (value) {
      if (value) {
        ...
        if (
          !this.isLongPressed &&
          !this.previewStarted &&
          engine.getValue("[PreviewDeck1]", "play")
        ) {
...
Roland707M.crossfader.setCurve = function (
  channel,
  control,
  value,
  _status,
...
Roland707M.crossfader.setReverse = function (
  channel,
  control,
  value,
  _status,
...
Roland707M.Deck = function (deckNumbers, offset) {
  ...
  this.keylock = new components.Button({
    ...
    shiftOffset: 1,
    outKey: "keylock",
    currentRangeIndex: Roland707M.tempoRange.indexOf(
      engine.getValue("[Channel" + deckNumbers + "]", "rateRange"),
    )
      ? Roland707M.tempoRange.indexOf(
          engine.getValue("[Channel" + deckNumbers + "]", "rateRange"),
        )
      : 0,
    unshift: function () {
  ...
  this.cue = new components.CueButton({
    ...
    input: function (channel, control, value, status, group) {
      components.CueButton.prototype.input.call(
        this,
        channel,
        control,
        value,
    ...
    unshift: function () {
      ...
      this.onLongPress = function () {
        if (engine.getValue(this.group, "sync_enabled")) {
          ...
          const value =
            engine.getValue(this.group, "sync_leader") === 2 ? 0 : 2;
  ...
  this.crossfaderOrientation = function (
    channel,
    control,
    value,
    status,
...
Roland707M.DeckToggleButton.prototype = Object.create(
  components.Button.prototype,
);
Roland707M.DeckToggleButton.prototype.input = function (
  channel,
  control,
  value,
  status,
...
Roland707M.DeckToggleButton.prototype.trigger = function () {
  ...
  const newGroup =
    "[Channel" + (this.secondaryDeck ? this.decks[1] : this.decks[0]) + "]";
...
Roland707M.Sampler = function () {
  ...
  this.startStopButtonPressed = function (
    channel,
    control,
    value,
    status,
  ...
  this.customSamplePlayback = function (
    channel,
    control,
    value,
    status,
    ...
    if (value) {
      ...
      engine.setValue(
        group,
        "volume",
        engine.getValue("[" + Roland707M.trsGroup + "]", "volume"),
      );
...
Roland707M.Sampler.prototype = Object.create(
...
Roland707M.SlipModeButton.prototype = Object.create(
  components.Button.prototype,
);
...
Roland707M.PadSection = function (deck, offset) {
  ...
  midi.sendShortMsg(
    0x94 + offset,
    this.modes.manualloop.ledControl,
    Roland707M.PadColor.OFF,
  );
  midi.sendShortMsg(
    0x94 + offset,
    this.modes.savedloop.ledControl,
    Roland707M.PadColor.OFF,
  );
  midi.sendShortMsg(
    0x94 + offset,
...
Roland707M.PadSection.prototype.controlToPadMode = function (control) {
  ...
  switch (control) {
    ...
    case Roland707M.PadMode.MANUALLOOP:
      ...
      break;
    case Roland707M.PadMode.SAVEDLOOP:
      mode = this.modes.savedloop;
      break;
    case Roland707M.PadMode.SLICER:
      // Slicer mode not implemented yet
      mode = null;
      break;
...
Roland707M.PadSection.prototype.padModeButtonPressed = function (
  channel,
  control,
  value,
  _status,
...
Roland707M.PadSection.prototype.paramButtonPressed = function (
  channel,
  control,
  value,
  status,
...
Roland707M.EditMode = function (deck, offset) {
  ...
  this.pads = new components.ComponentContainer();
  for (let i = 0; i <= 3; i++) {
    const baseKey = ["intro_start", "intro_end", "outro_start", "outro_end"][i];
...
Roland707M.EditMode.prototype = Object.create(
  components.ComponentContainer.prototype,
);
...
Roland707M.ManualLoopMode = function (deck, offset) {
  ...
  // Pads 1-4 (0x14-0x17 unshifted, 0x1C-0x1F shifted) - Saved Loop Cues (Hotcues 17-20)
  // Unshifted: Save/recall loops | Shifted: Clear hotcues
  for (var i = 0; i < 4; i++) {
    this.pads[i] = new components.Button({
      midi: [0x94 + offset, 0x14 + i],
      sendShifted: true,
        ...
        this.input = function (channel, control, value, status, group) {
          if (value > 0) {
            ...
            if (isHotcueSet) {
              ...
              // Save current loop as hotcue (or set regular cue if no loop active)
              engine.setValue(group, "hotcue_" + hotcueNum + "_setloop", 1);
            }
          }
        };
  ...
  // UNSHIFTED MODE: Pads 5-8 (0x18-0x1B) - Loop Controls
  // Pad 5 (0x18) - Loop In
  ...
  this.paramMinusButton = new components.Button({
    ...
    type: components.Button.prototype.types.push,
    input: function (channel, control, value, status, group) {
      // Only trigger on press (non-zero), ignore release
      if (value > 0) {
        components.Button.prototype.input.call(
          this,
          channel,
...
Roland707M.ManualLoopMode.prototype = Object.create(
  components.ComponentContainer.prototype,
);
...
Roland707M.SavedLoopMode = function (deck, offset) {
  ...
  // All Pads 1-8 (0x14-0x1B) - Recall Saved Loop Cues (Hotcues 17-24, recall only)
  for (var i = 0; i < 8; i++) {
    this.pads[i] = new components.Button({
      midi: [0x94 + offset, 0x14 + i],
      number: i + 17, // Hotcues 17-24
      ...
      input: function (channel, control, value, status, group) {
        if (value > 0) {
          ...
          if (isHotcueSet) {
            // Jump to hotcue and activate loop (recall only)
            engine.setValue(group, "hotcue_" + hotcueNum + "_gotoandloop", 1);
          }
          // Note: Does not save loops - use Manual Loop mode pads 1-4 to save
        }
      },
      output: function (value, group, control) {
        // Light up if hotcue is set
        this.send(value ? padColor : Roland707M.PadColor.OFF);
      },
      outConnect: true,
      outKey: "hotcue_" + (i + 17) + "_enabled",
    });
  ...
  // Parameter buttons could be used for additional functions if needed
  this.paramMinusButton = new components.Button({
    midi: [0x94 + offset, 0x28],
    group: deck.currentDeck,
  });
...
Roland707M.SavedLoopMode.prototype = Object.create(
  components.ComponentContainer.prototype,
);
    ...
    outputColor: function (colorCode) {
      ...
      this.send(
        this.mode.cuepoint === this.number
          ? colorValue
          : colorValue + Roland707M.PadColor.DIM_MODIFIER,
      );
    },
    unshift: function () {
      ...
      this.output = function (_value, _group, _control) {
        ...
        if (
          (this.mode.range === PitchPlayRange.UP && this.number === 5) ||
      ...
      this.input = function (channel, control, value, _status, _group) {
        if (value > 0) {
          ...
          engine.setValue(
            this.group,
            "hotcue_" + this.mode.cuepoint + "_activate",
            value,
          );
        }
      };
    ...
    shift: function () {
      ...
      this.input = function (channel, control, value, _status, _group) {
        if (
          value > 0 &&
          this.mode.cuepoint !== this.number &&
          engine.getValue(this.group, "hotcue_" + this.number + "_enabled")
        ) {
...
Roland707M.PitchPlayMode.prototype = Object.create(
  components.ComponentContainer.prototype,
);
