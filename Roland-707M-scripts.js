/*
 * The Roland 707M controller has two basic modes:
 *
 * 1. Standalone mode
 *
 * When the 707M is not connected to a computer (or no SysEx/Keep-Alive
 ...
 * 2. Serato mode
 *
 * When the 707M receives a SysEx message, the controller is put in "Serato"
 * mode.  However, in order to keep the 707M in this mode, it seems to be
 ...
 *
 *
 * Other quirks and issues of the Roland 707M:
...
Roland707M.init = function() {
    Roland707M.shiftButton = function(channel, control, value, _status, _group) {
        Roland707M.deck.concat(Roland707M.effectUnit, [Roland707M.sampler, Roland707M.browseEncoder]).forEach(
    ...
    Roland707M.deck = [];
    for (let i = 0; i < 4; i++) {
        Roland707M.deck[i] = new Roland707M.Deck(i + 1, i);
        Roland707M.deck[i].setCurrentDeck("[Channel" + (i + 1) + "]");
    }
    ...
    Roland707M.deck3Button = new Roland707M.DeckToggleButton({
        midi: [0x90, 0x08],
        decks: [1, 3],
        loadTrackButton: Roland707M.leftLoadTrackButton,
    });
    ...
    Roland707M.deck4Button = new Roland707M.DeckToggleButton({
        midi: [0x91, 0x08],
        decks: [2, 4],
        loadTrackButton: Roland707M.rightLoadTrackButton,
    });

    Roland707M.sampler = new Roland707M.Sampler();

    Roland707M.effectUnit = [];
    for (let i = 0; i <= 1; i++) {
        Roland707M.effectUnit[i] = new components.EffectUnit([i + 1, i + 3]);
        Roland707M.effectUnit[i].sendShifted = true;
        Roland707M.effectUnit[i].shiftOffset = 0x0B;
        Roland707M.effectUnit[i].shiftControl = true;
        Roland707M.effectUnit[i].enableButtons[1].midi = [0x98 + i, 0x00];
        Roland707M.effectUnit[i].enableButtons[2].midi = [0x98 + i, 0x01];
        Roland707M.effectUnit[i].enableButtons[3].midi = [0x98 + i, 0x02];
        Roland707M.effectUnit[i].effectFocusButton.midi = [0x98 + i, 0x04];
        Roland707M.effectUnit[i].knobs[1].midi = [0xB8 + i, 0x00];
        Roland707M.effectUnit[i].knobs[2].midi = [0xB8 + i, 0x01];
        Roland707M.effectUnit[i].knobs[3].midi = [0xB8 + i, 0x02];
        Roland707M.effectUnit[i].dryWetKnob.midi = [0xB8 + i, 0x03];
        Roland707M.effectUnit[i].dryWetKnob.input = function(channel, control, value, _status, _group) {
            if (value === 1) {
        ...
        for (let j = 1; j <= 4; j++) {
            Roland707M.effectUnit[i].enableOnChannelButtons.addButton("Channel" + j);
            Roland707M.effectUnit[i].enableOnChannelButtons["Channel" + j].midi = [0x98 + i, 0x04 + j];
        }
        Roland707M.effectUnit[i].enableOnChannelButtons.addButton(Roland707M.trsGroup);
        Roland707M.effectUnit[i].enableOnChannelButtons[Roland707M.trsGroup].midi = [0x98 + i, 0x09];
        Roland707M.effectUnit[i].enableOnChannelButtons[Roland707M.trsGroup].input = function(channel, control, value, status, _group) {
            components.Button.prototype.input.apply(this, arguments);
        ...
        Roland707M.effectUnit[i].enableOnTrsButton = Roland707M.effectUnit[i].enableOnChannelButtons[Roland707M.trsGroup];
        Roland707M.effectUnit[i].init();
    }

    engine.makeConnection("[Channel3]", "track_loaded", Roland707M.autoShowDecks);
    engine.makeConnection("[Channel4]", "track_loaded", Roland707M.autoShowDecks);
    ...
    Roland707M.deck4Button.trigger();
    for (let i = 0; i < 4; i++) {
...
Roland707M.autoShowDecks = function(_value, _group, _control) {
    ...
    if (!Roland707M.autoShowFourDecks) {
        return;
    }
...
Roland707M.sortLibrary = function(channel, control, value, _status, _group) {
    if (value === 0) {
        return;
    }
...
Roland707M.crossfader = new components.Pot({
    midi: [0xBF, 0x08],
    group: "[Master]",
    inKey: "crossfader",
    input: function() {
...
Roland707M.crossfader.setCurve = function(channel, control, value, _status, _group) {
...
Roland707M.crossfader.setReverse = function(channel, control, value, _status, _group) {
    // 0x00 is ON, 0x01 is OFF
    engine.setValue("[Mixer Profile]", "xFaderReverse", (value === 0x00) ? 1 : 0);
};

Roland707M.setChannelInput = function(channel, control, value, _status, _group) {
...
Roland707M.Deck = function(deckNumbers, offset) {
    ...
    this.wheelTurn = function(channel, control, value, _status, _group) {
        ...
        if (engine.isScratching(deck)) {
            ...
            const newPos = Math.max(0, oldPos + (newValue * Roland707M.stripSearchScaling / duration));
    ...
    this.padSection = new Roland707M.PadSection(this, offset);
        ...
        unshift: function() {
            ...
            this.input = function(channel, control, value, status, _group) {
                if (this.isPress(channel, control, value, status)) {
                    ...
                    if (this.currentRangeIndex >= Roland707M.tempoRange.length) {
...
Roland707M.DeckToggleButton = function(options) {
    this.secondaryDeck = false;
    components.Button.call(this, options);
};
Roland707M.DeckToggleButton.prototype = Object.create(components.Button.prototype);
Roland707M.DeckToggleButton.prototype.input = function(channel, control, value, status, _group) {
    if (this.isPress(channel, control, value, status)) {
        // Button was pressed
        this.longPressTimer = engine.beginTimer(
            this.longPressTimeout,
...
Roland707M.DeckToggleButton.prototype.trigger = function() {
...
Roland707M.Sampler = function() {
     ...
     * messages are sent by the 707M when the sampler step is reached:
     *
     *   9F 2X YY
     *   8F 2X 00
     *
    ...
    this.customSamplePlayback = function(channel, control, value, status, group) {
        if (value) {
            ...
            engine.setValue(group, "volume", engine.getValue("[" + Roland707M.trsGroup + "]", "volume"));
            engine.setValue(group, "cue_gotoandplay", 1);
        }
    };

    this.levelKnob = new components.Pot({
        group: "[" + Roland707M.trsGroup + "]",
        inKey: "volume",
...
Roland707M.Sampler.prototype = Object.create(components.ComponentContainer.prototype);
...
Roland707M.SlipModeButton = function(options) {
    components.Button.apply(this, arguments);
    this.doubleTapTimeout = 500;

    components.Button.call(this, options);
};
Roland707M.SlipModeButton.prototype = Object.create(components.Button.prototype);
Roland707M.SlipModeButton.prototype.unshift = function() {
...
Roland707M.SlipModeButton.prototype.shift = function() {
    this.input = components.Button.prototype.input;
...
Roland707M.PadColorMap = new ColorMapper({
    0xCC0000: Roland707M.PadColor.RED,
    0xCC4400: Roland707M.PadColor.CORAL,
    0xCC8800: Roland707M.PadColor.ORANGE,
    0xCCCC00: Roland707M.PadColor.YELLOW,
    0x88CC00: Roland707M.PadColor.GREEN,
    0x00CC00: Roland707M.PadColor.APPLEGREEN,
    0x00CC88: Roland707M.PadColor.AQUAMARINE,
    0x00CCCC: Roland707M.PadColor.TURQUOISE,
    0x0088CC: Roland707M.PadColor.CELESTE,
    0x0000CC: Roland707M.PadColor.BLUE,
    0x4400CC: Roland707M.PadColor.AZURE,
    0x8800CC: Roland707M.PadColor.PURPLE,
    0xCC00CC: Roland707M.PadColor.MAGENTA,
    0xCC0044: Roland707M.PadColor.RED,
    0xFFCCCC: Roland707M.PadColor.APRICOT,
    0xFFFFFF: Roland707M.PadColor.WHITE,
});

Roland707M.PadSection = function(deck, offset) {
    // TODO: Add support for missing modes (flip, slicer, slicerloop)
    /*
     * The Performance Pad Section on the 707M apparently have two basic
    ...
    this.modes = {
        ...
        "edit": new Roland707M.EditMode(deck, offset),
        "roll": new Roland707M.RollMode(deck, offset),
        "sampler": new Roland707M.SamplerMode(deck, offset),
        "velocitysampler": new Roland707M.VelocitySamplerMode(deck, offset),
        "loop": new Roland707M.SavedLoopMode(deck, offset),
    ...
    midi.sendShortMsg(0x94 + offset, this.modes.roll.ledControl, Roland707M.PadColor.OFF);
    midi.sendShortMsg(0x94 + offset, this.modes.sampler.ledControl, Roland707M.PadColor.OFF);
};

Roland707M.PadSection.prototype = Object.create(components.ComponentContainer.prototype);

Roland707M.PadSection.prototype.controlToPadMode = function(control) {
    let mode;
    switch (control) {
    ...
    case Roland707M.PadMode.CUELOOP:
        ...
        break;
    case Roland707M.PadMode.TR:
    case Roland707M.PadMode.PATTERN:
    case Roland707M.PadMode.TRVELOCITY:
...
Roland707M.PadSection.prototype.padModeButtonPressed = function(channel, control, value, _status, _group) {
    if (value) {
        this.setPadMode(control);
    }
};

Roland707M.PadSection.prototype.paramButtonPressed = function(channel, control, value, status, group) {
    if (!this.currentMode) {
        return;
    }
    let button;
...
Roland707M.PadSection.prototype.setPadMode = function(control) {
...
Roland707M.PadSection.prototype.padPressed = function(channel, control, value, status, group) {
...
Roland707M.HotcueMode = function(deck, offset) {
    components.ComponentContainer.call(this);
    this.ledControl = Roland707M.PadMode.HOTCUE;
    this.color = Roland707M.PadColor.WHITE;
...
Roland707M.HotcueMode.prototype = Object.create(components.ComponentContainer.prototype);
...
Roland707M.CueLoopMode.prototype = Object.create(components.ComponentContainer.prototype);

Roland707M.EditMode = function(deck, offset) {
    components.ComponentContainer.call(this);
    this.ledControl = Roland707M.PadMode.HOTCUE;
    this.color = Roland707M.PadColor.RED;
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        const color = (i > 1) ? Roland707M.PadColor.AZURE : Roland707M.PadColor.BLUE;
        ...
        this.pads[i] = new components.Button({
            ...
            group: deck.currentDeck,
            on: color,
            off: color + Roland707M.PadColor.DIM_MODIFIER,
            outConnect: false,
            unshift: function() {
...
Roland707M.EditMode.prototype = Object.create(components.ComponentContainer.prototype);
...
Roland707M.RollMode = function(deck, offset) {
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        this.pads[i] = new components.Button({
            ...
            outConnect: false,
            on: this.color,
            off: (loopSize === 0.25) ? Roland707M.PadColor.TURQUOISE : ((loopSize === 4) ? Roland707M.PadColor.AQUAMARINE : (this.color + Roland707M.PadColor.DIM_MODIFIER)),
        });
    }
...
Roland707M.RollMode.prototype = Object.create(components.ComponentContainer.prototype);
Roland707M.RollMode.prototype.setLoopSize = function(loopSize) {
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        this.pads[i].off = (padLoopSize === 0.25) ? Roland707M.PadColor.TURQUOISE : ((padLoopSize === 4) ? Roland707M.PadColor.AQUAMARINE : (this.color + Roland707M.PadColor.DIM_MODIFIER));
    }
            ...
            if (value === 2) {
                this.connections[2] = engine.makeConnection("[App]", "indicator_250ms", function(value, _group, _control) {
                    ...
                    if (value) {
                        ...
                        this.send(colorValue + Roland707M.PadColor.DIM_MODIFIER);
                    }
                }.bind(this));
            } else if (value === 1) {
                const colorValue = this.colorMapper.getValueForNearestColor(
                ...
                this.send(this.on + Roland707M.PadColor.DIM_MODIFIER);
            }
        },
        connect: function() {
...
Roland707M.SavedLoopMode.prototype = Object.create(components.ComponentContainer.prototype);

Roland707M.SamplerMode = function(deck, offset) {
    components.ComponentContainer.call(this);
    this.ledControl = Roland707M.PadMode.SAMPLER;
    this.color = Roland707M.PadColor.MAGENTA;
    this.pads = new components.ComponentContainer();
    for (let i = 0; i <= 7; i++) {
...
Roland707M.SamplerMode.prototype = Object.create(components.ComponentContainer.prototype);
...
Roland707M.VelocitySamplerMode.prototype = Object.create(components.ComponentContainer.prototype);

Roland707M.PitchPlayMode = function(deck, offset) {
    components.ComponentContainer.call(this);
    ...
    this.PerformancePad = function(n) {
        this.midi = [0x94 + offset, 0x14 + n];
        this.number = n + 1;
        this.on = this.color + Roland707M.PadColor.DIM_MODIFIER;
        this.colorMapper = Roland707M.PadColorMap;
        this.colorKey = "hotcue_" + this.number + "_color";
        components.Button.call(this);
    };
    this.PerformancePad.prototype = new components.Button({
        ...
        outputColor: function(colorCode) {
            ...
            this.send((this.mode.cuepoint === this.number) ? colorValue : (colorValue + Roland707M.PadColor.DIM_MODIFIER));
        },
        unshift: function() {
            this.outKey = "pitch_adjust";
            this.output = function(_value, _group, _control) {
                let color = this.mode.color + Roland707M.PadColor.DIM_MODIFIER;
                if ((this.mode.range === PitchPlayRange.UP && this.number === 5) ||
                    ...
                    color = Roland707M.PadColor.WHITE;
                }
                this.send(color);
            };
            this.input = function(channel, control, value, _status, _group) {
        ...
        shift: function() {
            ...
            this.output = function(value, _group, _control) {
                ...
                if (this.colorKey !== undefined && outval !== this.off) {
                    ...
                    this.send(Roland707M.PadColor.OFF);
                }
            };
            this.input = function(channel, control, value, _status, _group) {
...
Roland707M.PitchPlayMode.prototype = Object.create(components.ComponentContainer.prototype);
